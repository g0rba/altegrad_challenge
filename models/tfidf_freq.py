...
class LDAClassifier(BaseEstimator):
        # We experimented multiple value using PCA and it ends up 16 is the best value
        # See http://onlinelibrary.wiley.com/doi/10.1002/jrs.4908/full
        def __init__(self):
...
class TfIdfFreqModel(AbstractModel):
    ...
    def __init__(self, nb_recipients_to_predict=10, use_customed_tokens=True, agg_func='mean', ngram_max=2, verbose=1, use_pretrained_model=False):
        ...
        if self.use_pretrained_model:
            ...
            for idx, row in training.iterrows():
                ...
                for mid_predict in mids_predict:                
                    ...
                    tfidf_mails_sender = self.training_matrix[np.setdiff1d(position_mails_training,position_mail_predict)]
    ...
    def predict(self, test, test_info, training, training_info):
